...
class Qwen2VLAttention(nn.Module):
    ...
    def forward(
        ...
        if use_add is True:
            ...
            attn_weights_sf = nn.functional.softmax(attn_weights_cur, dim=-1, dtype=torch.float32)

            attn_weights_v = attn_weights_sf[:, :, :, img_idx[0]: img_idx[1]]
            attn_weights_t = attn_weights_sf[:, :, :, img_idx[1]:]

            sum_v = attn_weights_v.sum(dim=-1)
            sum_t = attn_weights_t.sum(dim=-1)

            text_head = (sum_v - sum_t) <= 0
            vision_head = ~text_head

            global_head = (attn_weights_t.max(dim=-1).values <= sum_t*kappa)
            local_head = ~global_head

            global_head_v = global_head * vision_head
            local_head_v = local_head * vision_head
            global_head_t = global_head * text_head
            local_head_t = local_head * text_head

            coef = omega
            for b in range(1):
                if min(vision_head[b].sum(),global_head_t[b].sum()) > 0:
                    attn_weights_cur[b,vision_head[b]] += coef * attn_weights_cur[b, global_head_t[b]].sum(dim=0)/global_head_t[b].sum()
                    attn_weights_cur[b,vision_head[b]] /=(1+coef)
                if min(local_head_t[b].sum(),global_head_t[b].sum()) > 0:
                    attn_weights_cur[b,global_head_t[b]] += coef * attn_weights_cur[b, local_head_t[b]].max(dim=0).values.squeeze(0)
                    attn_weights_cur[b,global_head_t[b]] /=(1+coef)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    kappa=kappa,
                    omega=omega,
                    use_add=use_add and (idx in add_layer),
                    img_idx=img_idx,
                )
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_token_indices = torch.where(input_ids[0] == self.config.image_token_id)[0]
                start_pos = image_token_indices[0].item()
                end_pos = image_token_indices[-1].item()
                img_idx = (start_pos, end_pos+1, input_ids.size(-1))
    ...
    def prepare_inputs_for_generation_cd(
        ...
        **kwargs,
        ...
        # model_inputs['input_ids']=kwargs.get('input_ids_cd')
        # model_inputs['mask']=kwargs.get('pixel_values_cd')
            model_inputs['pixel_values']=kwargs.get('pixel_values_cd')
            model_inputs['image_grid_thw']=kwargs.get('image_grid_thw_cd')
        return model_inputs
