...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        if self.layer_idx >=9 and self.layer_idx <=15:
        # if self.layer_idx >=13 and self.layer_idx <=21:
            attn_weights_cur = attn_weights[:,:,-1,:]
            
            attn_vis = attn_weights_cur[:,:,img_idx[0]:img_idx[1]].max(1).values

            attn_weights_cur[:,:,img_idx[0]:img_idx[1]] += 0.5*attn_vis
...
# ClearSight
class Qwen2_5_VLAttention_vaf(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """
    ...
    def forward(
        ...
        if self.layer_idx >8 and self.layer_idx<15:
            sup_para=0.95
            enh_para=1.15
            sys_len = img_idx[0]
            img_len = img_idx[1]-sys_len
