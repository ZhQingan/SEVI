...
# tokenizer = AutoTokenizer.from_pretrained("/path/to//models/llava-vicuna-v1.5/llava-v1.5-7b", use_fast=False)
...
def sample(
    ...
    **model_kwargs,
    ...
    cross_jsd_th = model_kwargs.get("cross_jsd_th")
    ...
    while True:
        ...
        if use_cd:
            ...
            # if version > 0:
            #     probs = nn.functional.softmax(next_token_logits * version, dim=-1)
            ...
            m2 = 0.5 * (nn.functional.softmax(next_token_logits, dim=-1, dtype=torch.float32) + nn.functional.softmax(next_token_logits_cd, dim=-1, dtype=torch.float32))
            jsd_cross = 0.5 * nn.functional.kl_div(nn.functional.log_softmax(next_token_logits, dim=-1, dtype=torch.float32), m2, reduction='batchmean') + 0.5 * nn.functional.kl_div(nn.functional.log_softmax(next_token_logits_cd, dim=-1, dtype=torch.float32), m2, reduction='batchmean')
            # alpha = 3
            jsd_cross = min(torch.tensor(1.),jsd_cross)
            alpha = min(3, 1-torch.log10((jsd_cross)))
            if jsd_cross > cross_jsd_th:
